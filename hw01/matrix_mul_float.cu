...
__global__ void mmul_float(const float* A, const float* B, float* C, int ds) {
...
int main_matrix_mul_float() {
    ...
    printf("gx: %d, gy: %d\n", gx, gy);
    dim3 grid(gx, gy);
    mmul_float << <grid, block >> > (d_A, d_B, d_C, DSIZE);
    ...
    for (int i = 0; i < DSIZE * DSIZE; i++)
    {
        ...
        float avg = static_cast<float>(b * DSIZE) + static_cast<float>(spread) / 2.0f;
        float sA = DSIZE * avg;
        float s = sA * B_val;
        if (b != last_b)
        {
